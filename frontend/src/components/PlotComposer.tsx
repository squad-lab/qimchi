    ...
    try {
      ...
      if (invalidFields.length > 0) {
        ...
        if (field === "x" && xFields.length >= 1) {
          errorMessage = "X-axis can only accept one field";
        } else if (field === "z" && plotType !== "HeatMap") {
          errorMessage = "Z-axis is only available for HeatMap plots";
        } else if (plotType === "LinePlot") {
          if (field === "y" && fieldType === "independent") {
            errorMessage = "LinePlot Y-axis only accepts dependent variables";
          }
        } else if (plotType === "HeatMap") {
          if (field === "z" && fieldType === "independent") {
            errorMessage = "HeatMap Z-axis only accepts dependent variables";
          }
        }
                ...
                content={
                  <div className="text-left space-y-2">
                    <div className="text-sm text-white">
                      Drag indeps and deps from the basket items to create
                      plots.
                    </div>
                  </div>
                }
